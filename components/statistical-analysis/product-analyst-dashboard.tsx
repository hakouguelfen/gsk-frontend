...
import { MedicationOOSHistogram } from "./medication-oos";
import { MedicationQuantitiesLineChart } from "./medication-quantity";

interface ProductionAnalystDashboardProps {
  data: any[];
}

export function ProductionAnalystStatsDashboard({
  data,
}: ProductionAnalystDashboardProps) {
  const [selectedProduct, setSelectedProduct] = useState("all");
  const [selectedLot, setSelectedLot] = useState("all");

  // Calculate process performance metrics with validation
  const metrics = useMemo(() => {
    const activeIngredients = data.map((d) =>
      validateNumber(d?.activeIngredientQty),
    );
  ...
  const productPerformance = useMemo(() => {
    const productStats = data.reduce(
      (acc, d) => {
        ...
        if (d?.activeIngredientQty)
          acc[productType].activeIngredients.push(
            validateNumber(d.activeIngredientQty),
          );
        if (d?.moistureContent)
          acc[productType].moistures.push(validateNumber(d.moistureContent));
        if (d?.granulePorosity)
          acc[productType].porosities.push(validateNumber(d.granulePorosity));
    ...
    return Object.entries(productStats).map(([type, stats]) => ({
      ...
      avgActiveIngredient: formatNumber(
        safeAverage(stats.activeIngredients),
        1,
      ),
  ...
  const processControlData = useMemo(() => {
    ...
    const sortedData = [...data].sort((a, b) => {
      const dateA =
        a?.dateOccurrence instanceof Date
          ? a.dateOccurrence
          : new Date(a?.dateOccurrence || 0);

      const dateB =
        b?.dateOccurrence instanceof Date
          ? b.dateOccurrence
    ...
    const activeIngredients = recentData.map((d) =>
      validateNumber(d?.activeIngredientQty),
    );
    const avgActiveIngredient = safeAverage(activeIngredients);
    const stdDevActiveIngredient =
      Math.sqrt(
        activeIngredients.reduce(
          (sum, a) => sum + Math.pow(a - avgActiveIngredient, 2),
          0,
        ) / activeIngredients.length,
      ) || 1; // Prevent NaN

    const uclActiveIngredient =
      avgActiveIngredient + 3 * stdDevActiveIngredient;
    const lclActiveIngredient =
      avgActiveIngredient - 3 * stdDevActiveIngredient;
  ...
  const psdAnalysisData = useMemo(() => {
    ...
    const psdDistribution = [
      { size: "0-25 µm", count: d10Values.filter((v) => v < 25).length },
      {
        size: "25-50 µm",
        count:
          d10Values.filter((v) => v >= 25 && v < 50).length +
          d50Values.filter((v) => v < 50).length,
      },
      {
        size: "50-75 µm",
        count: d50Values.filter((v) => v >= 50 && v < 75).length,
      },
      {
        size: "75-100 µm",
        count:
          d50Values.filter((v) => v >= 75 && v < 100).length +
    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card className="col-span-1">
          <CardHeader>
            <CardTitle className="text-lg font-semibold text-gray-800">
              Médicaments: Excipients vs Principes Actifs
              <span className="text-sm font-normal text-gray-500 ml-2">
