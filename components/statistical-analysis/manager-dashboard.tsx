...
import { OOSEvolutionChart } from "./oos-evolution-chart";
import { LeadTimeByRootCauseChart } from "./leadtime-rootcause-chart";
...
export function ManagerDashboard({ data, dateRange }: ManagerDashboardProps) {
  ...
  const kpis = useMemo(() => {
    ...
    const oosRecords = data.filter((d) =>
      d?.oosOotType?.includes("OOS"),
    ).length;
  ...
  const productPerformance = useMemo(() => {
    const productStats = data.reduce(
      (acc, d) => {
        ...
        if (d?.leadTime)
          acc[product].leadTimes.push(validateNumber(d.leadTime));
  ...
  const equipmentRiskData = useMemo(() => {
    const equipmentData = data.reduce(
      (acc, d) => {
        ...
        if (d?.equipmentAge)
          acc[equipmentId].ages.push(validateNumber(d.equipmentAge));
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {Number.parseFloat(kpis.oosRate) > 5
                ? "Above target"
                : "Within target"}
            </p>
          </CardContent>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">OOT Rate</CardTitle>
            <AlertTriangle className="h-4 w-4 text-red-500" />
          </CardHeader>
          <CardContent>
      ...
      <Tabs defaultValue="trends" className="space-y-4">
        ...
        <TabsContent value="trends" className="space-y-4">
          <Card className="col-span-1">
            <CardHeader>
              <CardTitle className="text-lg font-semibold text-gray-800">
                Évolution du nombre d'OOS par mois
              </CardTitle>
            </CardHeader>
            <CardContent>
              <OOSEvolutionChart />
          ...
          <Card className="col-span-1">
            <CardHeader>
              <CardTitle className="text-lg font-semibold text-gray-800">
                Lead Time by Root Cause
              </CardTitle>
            </CardHeader>
            <CardContent>
              <LeadTimeByRootCauseChart />
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="pareto" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Most frequent root causes requiring attention
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid md:grid-cols-2 gap-6">
                <div className="h-[300px]">
                  {rootCause && (
                    <ChartErrorBoundary>
                      <PieChart
                        labels={Object.entries(rootCause).map(
                          ([cause, _]) => cause,
                        )}
                        data={Object.entries(rootCause).map(([_, count]) =>
                          Number(count),
                        )}
                ...
                <div className="space-y-3">
                  {rootCause &&
                    Object.entries(rootCause).map(
                      ([cause, countStr], index) => {
                        const count = Number(countStr);
                        const total = Object.values(rootCause).reduce(
