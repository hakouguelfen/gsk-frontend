...
import { getCombinedBatchData, getModelFeedback } from "@/lib/firebase/firestore"
import { loadPredictions, predictRootCause } from "./load"
import { BatchDataViewer } from "./batch-data-viewer"
import { savePrediction } from "./server"
...
function RootCauseAnalysisContent() {
  ...
  const handleBatchSearch = async (batch: string) => {
    ...
    try {
      setIsSearching(true)
      const data = await getCombinedBatchData(batch)
      ...
      if (detectedProduct && user) {
        setIsAnalyzing(true)

        try {
          let prediction = await predictRootCause(batch)
          prediction.product = detectedProduct
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        <span
                          className={`px-2 py-1 rounded text-xs font-medium ${
                            feedback.userFeedback.isCorrect ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                          }`}
                        >
