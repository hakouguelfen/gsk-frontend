...
import { sendNotification, getNotifications, markNotificationAsRead, type Notification } from "@/lib/firebase/firestore"
...
export default function ProductionNotifications() {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      setSentNotifications(allNotifications.filter((n) => n.sender === user.uid))
  ...
  // Add a useEffect to show a message about the required index
  ...
  const handleSendNotification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (recipient === "lab-team") {
        recipients = ["lab-team-id"]
      } else if (recipient === "quality-manager") {
        recipients = ["quality-manager-id"]
      } else if (recipient === "production-manager") {
        recipients = ["production-manager-id"]
      } else if (recipient === "all") {
        recipients = ["lab-team-id", "quality-manager-id", "production-manager-id"]
      }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Send and receive notifications related to production processes and quality control.
        </p>
      </div>
      {hasError && (
        <Alert className="mb-6 bg-amber-50 border-amber-200">
          ...
          <AlertDescription>
            <p>This feature requires a Firestore index. Check the console for the link to create it.</p>
            <p className="text-xs text-muted-foreground mt-2">
              The app will continue to function with limited notification capabilities.
            </p>
          </AlertDescription>
        </Alert>
      )}
          ...
          <Card>
            <form onSubmit={handleSendNotification}>
              <CardHeader>
                ...
                <CardDescription>Send notifications to laboratory team or management.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select value={recipient} onValueChange={setRecipient} required>
                      ...
                      <SelectContent>
                        <SelectItem value="lab-team">Laboratory Team</SelectItem>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="subject"
                    placeholder="e.g., Production Complete for Augmentin Batch A2023-52"
                    value={subject}
        ...
        <TabsContent value="inbox">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Notifications received from other teams.</CardDescription>
            </CardHeader>

            <CardContent>
              {notifications.length === 0 ? (
                <div className="flex items-center justify-center h-40 bg-gray-50 rounded-md">
                  <p className="text-muted-foreground">No notifications received</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {notifications.map((notification) => (
                    <div key={notification.id} className="flex items-center justify-between rounded-md border p-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <Bell className="h-4 w-4" />
                          <span className="font-medium">{notification.subject}</span>
                          {!notification.read && <Badge>New</Badge>}
                          {notification.priority === "high" && <Badge variant="destructive">Urgent</Badge>}
                        </div>
                        <p className="text-sm text-muted-foreground">{notification.message}</p>
                        <div className="text-xs text-muted-foreground">
                          From: {notification.sender} • {notification.createdAt?.toDate().toLocaleString()}
                        </div>
                      </div>
                      <Button
                        variant="outline"
        ...
        <TabsContent value="sent">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Notifications you have sent to other teams.</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  {sentNotifications.map((notification) => (
                    <div key={notification.id} className="flex items-center justify-between rounded-md border p-4">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm text-muted-foreground">{notification.message}</p>
                        <div className="text-xs text-muted-foreground">
                          To: {notification.recipients.join(", ")} • {notification.createdAt?.toDate().toLocaleString()}
                        </div>
                      </div>
